...
This laboratory investigates more closely the dead time behavior of a GM detector, comparing its response to sources of different strength and decay rates.
...
Thus no matter how rapidly events occur, the detector will never become oversaturated or register events faster than $1/\tau$, where $\tau$ is the dead time of the detector.
...
As previewed in Figure~\ref{fig:mat-setup}
...
These estimates are highly different from the dead time observed using the oscilloscope trace in Figure~\ref{fig:exp2-osc-dead}, which we estimated to be $0.562\pm0.002$~ms.

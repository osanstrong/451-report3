...
\usepackage{xcolor}
\usepackage[colorlinks=true, citecolor=violet, linkcolor=black, urlcolor=violet]{hyperref}
...
\begin{figure}
    ...
    \caption{Traces #LABEL ON FIGURE# (faded yellow) of signals (bright yellow) from the GM Detector while exposed to the 30~\micCi~beta source.
...
Even judging conservatively, assuming that pulses would not be properly counted until 95\% recovery, this would yield a recovery time from the same trace of Figure~\ref{fig:exp2-osc-dead} of $1.384\pm0.002$~ms, still nearly an order of magnitude different than that calculated by the two source method.\\
